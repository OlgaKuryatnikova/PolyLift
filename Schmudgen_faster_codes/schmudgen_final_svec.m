...
for j=1:lenG
    ...
    % check if g is a bound polynomial
    if numMong(j)<=2 && Dg(j)==1
        [~,ind_bound] = find(g(j).degmat);
        coefg = g(j).coef;
        if length(ind_bound) == 1
            if numMong(j) == 1 && coefg(1) > 0 % ax >= 0
                L(ind_bound) = 0;
                vec_bounds(ind_bound,1) = 1;
            elseif numMong(j) == 1 && coefg(1) < 0 % -ax >= 0
                U(ind_bound) = 0;
                vec_bounds(ind_bound,2) = 1;
            elseif numMong(j) == 2 && coefg(1) > 0 % ax - L >= 0
                L(ind_bound) = -coefg(end)/coefg(1);
                vec_bounds(ind_bound,1) = 1;
            else % numMong(j) == 2 && coefg(end) < 0: U - ax >= 0
                U(ind_bound) = -coefg(end)/coefg(1);
                vec_bounds(ind_bound,2) = 1;
            end
        end
    end
...
monVecG = indexCreationLess(lenG,dmax); % sparse, can be crucial, inside the code can be changed to be full
indTotal = monVecG*Dg;
...
powers=uint64(dmax+1).^uint64((1:numVars)'-1);  % base to construct indices for comparison

% create all monomials for dmax and numVars variables
[~, numMon, codesFull] = add_monomials_and_codes(numVars,powers,[],dmax);
...
ML = add_coef_matr(1,dmax,0,numVars,powers,codesFull,0);
% add the right-hand side for f-lambda = certificate, f
MF = add_coef_matr(f,dmax,0,numVars,powers,codesFull,0);
...
if isempty(h)
    ...
    for rr=1:length(h)
        [Meq{rr},~,sizeq(rr)] = add_coef_matr(h(rr),dmax,[],numVars,powers,codesFull,0);
